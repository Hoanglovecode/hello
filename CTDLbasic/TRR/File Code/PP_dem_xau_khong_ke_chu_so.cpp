 ...
 * ĐIỀU KIỆN: KHÔNG CÓ HAI CHỮ SỐ KỀ NHAU (NO TWO ADJACENT DIGITS)
 * 
 * Mô tả bài toán:
 *   Sinh và đếm các xâu ký tự S có độ dài n sao cho không có 2 chữ số nào đứng cạnh nhau.
...
bool is_valid(char next_char, int pos) {
    ...
    // Không cho phép 2 chữ số kề nhau
    if (is_digit(prev_char) && is_digit(next_char)) {
        return false;
    }
    return true;
}
...
long long count_no_adjacent_digits_dp(int length) {
...
long long count_no_adjacent_digits_optimized(int length) {
    if (length <= 0) return 0;
    ...
    for (int i = 2; i <= length; i++) {
        long long next_digit = 4 * end_letter;
        long long next_letter = 8 * (end_digit + end_letter);
...
int main() {
    cout << "=== KHONG CO HAI CHU SO KE NHAU ===\n";
    cout << "Nhap do dai n: ";
    if (!(cin >> n) || n < 1) {
    ...
    cout << "\n[Quy hoach dong] Tong so xau thoa man: " << count_no_adjacent_digits_optimized(n) << "\n\n";
...
/*
 * # TRẢ LỜI: Giải thích phương pháp đếm và liệt kê xâu ký tự không có hai chữ số kề nhau:
 * 1. Phương pháp Quay lui (Backtracking):
 *    - Thử điền từng ký tự từ tập X vào vị trí pos từ 0 đến n-1.
 *    - Tại vị trí pos, kiểm tra nếu ký tự trước đó (prev_char) và ký tự hiện tại (next_char) đều là chữ số (is_digit) thì không hợp lệ, bỏ qua.
 *    - Khi pos == n, tăng biến đếm và in ra nếu n <= 4.
 * 
 * 2. Phương pháp Quy hoạch động (Dynamic Programming):
 *    - dp[i][0] là số xâu độ dài i kết thúc bằng chữ số. Vì không cho phép hai chữ số đứng cạnh nhau, chữ số bắt buộc phải đứng sau chữ cái, nên dp[i][0] = 4 * dp[i-1][1].
 *    - dp[i][1] là số xâu độ dài i kết thúc bằng chữ cái. Vì chữ cái có thể đứng sau chữ cái hoặc chữ số, nên dp[i][1] = 8 * (dp[i-1][0] + dp[i-1][1]).
 *    - Kết quả là dp[n][0] + dp[n][1].
 *    - Dạng tối ưu O(1) sử dụng hai biến end_digit và end_letter để lưu các giá trị của bước trước, giúp giảm độ phức tạp không gian xuống O(1).
 */
