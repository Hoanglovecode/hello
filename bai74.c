/*
Bài 74. Viết chương trình có sử dụng hàm để nhập ma trận A cấp mxn (với m,n nhập từ
bàn phím):
a. Xuất ma trận A ra màn hình
b. Đưa các phần tử của từng hàng của ma trận lên đường chéo chính và tính
tổng các phần tử lớn nhất đó 
*/
#include <stdio.h>
void nhapham(int m,int n,int a[100][100]){
    for(int i=0;i<m;i++){
        for(int j=0;j<n;j++){
            printf("a[%d][%d]=",i,j);
            scanf("%d",&a[i][j]);
...
void xuatham(int m,int n,int a[100][100]){
    for(int i=0;i<m;i++){
        for(int j=0;j<n;j++){
            printf("%d ",a[i][j]);
        }
...
int main(){
    ...
    else printf("Đây không phải ma trận vuông!");
}